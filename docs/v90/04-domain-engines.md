...
type RescheduleReason = 'moveToToday' | 'moveToDate' | 'resume' | 'partialContinuation' | 'cancelSession';
...
- **A-1** `Scheduler` 02 §3 modül haritasında (`domain/program/`: ChallengeCalendar, TrainingSequence, PauseService, MissedWorkoutResolver) listelenmiyor; §6.2 ve §17'de kullanılıyor. Bu belge `domain/program/Scheduler` varsayar; §3 güncellenmeli.
- **A-2** 02 §6.3 "Bölüm I'e göre lineer ise `programs.sequence_wraps = 0`" ifadesi 03 ile çelişir: 03'te lineer/döngüsel bayrağı `program_templates.is_cyclic`, `sequence_wraps` ise "kaç kez başa döndü" sayacıdır. Bu belge `is_cyclic`'i bayrak, `sequence_wraps`'ı sayaç olarak kullanır; 02 düzeltilmeli.
- **A-3** `sequence_events.cause='manualAdjust'` 03'te tanımlı ama 02'de hiçbir akış üretmiyor; R88.6 "başka hiçbir yol sırayı ilerletemez" der. Karar gerekli: değer 03'ten kaldırılmalı ya da Program Settings'te açık, onaylı bir "Sırayı düzelt" eylemi (aktif plan yokken, ayrı fonksiyon) tanımlanmalı. Bu belge `advanceSequence()`'ten kabul etmez (`SequenceAdvanceCause`).
- **A-4** `scheduled_workouts.reschedule_reason='cancelSession'` 03'te var; 02 §6.3/§7.1 cancelSession'ı yerinde geri açma (`inProgress → planned`, yeni kayıt yok) olarak tanımlar. Bu belge 02'yi izler (T9), `cancelSession` değeri hiç üretilmez → 03'ten çıkarılmalı ya da 02 reschedule modeline çevrilmeli.
...
- **A-6** `pause()` guard'ları 02'de yok: aktif oturum varken veya karar bekleyen kısmi antrenman varken dondurma. Bu belge her ikisini reddeder (`ActiveSessionExistsError`, `PendingPartialDecisionError`); aksi halde R89.3 (dondurmada sıra ilerlemez) ile bitirme/karar akışı çatışır.
- **A-7** Karar verilmemiş kısmi antrenman (`partiallyCompleted` + `partial_decision IS NULL`; uygulama karar ekranında kapandı) 02'de ele alınmamış. Bu belge `ensurePlanned` için `pendingPartialDecision` engelini ve "Kısmi antrenman kararı bekliyor" kartını tanımlar; aksi halde aynı `sequence_index` için ikinci bir tam plan oluşurdu.
- **A-8** Açık dondurma (`end_date_key IS NULL`) için `pausedDays` sayımı 02 §6.1'de belirtilmemiş. Bu belge geçici bitiş = bugün (`end_date_key ?? today`) kuralını seçer; böylece `challengeDay` dondurma günü geri gitmez, monoton kalır ve kapalı aralık `[start, end)` tanımıyla tutarlıdır.
- **A-9** `planned_date_key ≠ today` olan planın başlatılması (kaçırılmış veya erken) 02'de tanımsız. Bu belge `markInProgress` içinde önce `reschedule(today, 'moveToToday')` uygular → değişmez: açık/tamamlanmış planın `planned_date_key` = oturumun `calendar_date_key` (override hariç). Alternatif (planı yerinde bırakmak) adherence haftasını kaydırır.
...
- **A-14** Geçmiş bir güne taşıma (`newDateKey < today`) 02'de yasaklanmamış; bu belge reddeder (`InvalidRescheduleDateError`), aksi halde plan anında tekrar `missed` olur.
- **A-15** 02 §6.2 sonraki planı "bugünden itibaren ilk uygun gün" der; bu, tamamlanan antrenmanın gününe ikinci plan koyar ve ertesi gün sahte "kaçırıldı" üretir. Bu belge tamamlama/`countAsDone` için `earliest = max(today, session.calendar_date_key + 1)`, skip/resume/bootstrap için `today` kuralını getirir.
- **A-16** `start_date_key` gelecekte olan program (henüz başlamadı) 02'de yok: `ensurePlanned` başlangıçtan önce plan yapmamalı (`earliest ≥ start_date_key`) ve `clamp(…, 1, 90)` "başlamadı"yı Day 1 gibi gösterir. Bu belge `phase` alanı ekler (R123).
- **A-17** Timezone doğuya seyahatte bugünün planı yerel gün atladığı için anında `missed` görünebilir (02 §5.5 ±1 gün "beklenen"). Kart metninin takvim günü değişimini açıklaması gerekir; UX kararı açık.
...
1. **`set_logs` UNIQUE vs. `set_index` yorumu (03 §1.6):** `UNIQUE (session_exercise_id, set_index, side)` `set_type` içermezken kolon yorumu "warmup ve working ayrı sayılır" der. Ayrı indeks uzayları (warmup 1..w ve working 1..n) bu UNIQUE'i ihlal eder. Bu bölüm tek artan indeks (warmup 1..w, working w+1..) varsayar; 03'teki yorum netleştirilmeli ya da UNIQUE'e `set_type` eklenmeli.
2. **`set_logs.local_date_key` aidiyeti:** 02 §5.1 genel kuralı "yazıldığı andaki yerel tarih" der; bu bölüm R113.1 için `local_date_key := workout_sessions.calendar_date_key` (oturum günü) ve `time_zone := kayıt anındaki tz` kullanır, `overrideCalendarDate` bunları oturumla birlikte taşır. 02 §5.1/§5.3'e bu istisna eklenmeli (`personal_records.local_date_key` için de aynı).
3. **İptalde PR geri alma:** 02 §7.1 iptalde setlerin `discarded=1` olduğunu söyler ama commit anında yazılmış `personal_records` için kural yok; 03'te void bayrağı yok. Bu bölüm zinciri onarıp satırı siler (`DELETE … WHERE session_id`). Tercihen `002` migration'ında `personal_records.voided` kolonu.
4. **`cancelSession` FSM'i:** 02 §6.3 `inProgress → planned` ("plan geri açılır") der; 03 ise `reschedule_reason='cancelSession'` değeri tanımlar (yeni plan satırı ima eder). Bu bölüm yerinde `status='planned'` kullanır; `'cancelSession'` değeri kullanılmaz — 02/03 hangisini istediğini netleştirmeli.
5. **Karar verilmemiş kısmi antrenman:** `partiallyCompleted ∧ partial_decision IS NULL` durumu (finish ile karar arasında kapanış) 02'de ele alınmamış; bu bölüm `AppBootstrap`'e `partialDecision` kartı ekler ve `decidePartial` komutunu türetir.
...
7. **`command_log` sonuç kolonu yok:** duplicate replay orijinal sonucu döndüremez; `completeSet` için `set_logs.command_id` ile bulunur, diğerleri `hydrate()`'e dayanır. İsteğe bağlı `result_json` kolonu (gelecek migration). Ayrıca `command_log` temizleme kuralı (örn. 30 günden eski satırlar) hiçbir belgede yok.
8. **Kolon adı farkı:** 02 §7.1 `draft_load/draft_reps/draft_rir`, 03 `draft_load_json` (RawLoad JSON). Bu bölüm 03'ü kullanır; 02 düzeltilmeli.
...
14. **Dondurulmuş programda aktif oturum:** §1 taslağı `PauseService.pause`'un aktif oturum varken dondurmayı reddettiğini ve `advanceSequence`'in `paused` programda `ProgramNotActiveError` fırlattığını tanımlar (bu bölüm buna uyar, E18). 02 §6.5'te bu guard yazılı değil; eklenmeli.
15. **`bodyweight_kg_snapshot` kaynağı:** 02 §8.4 alanı tanımlar ama `start`'ta hangi `weight_logs` kaydının (yaş sınırı?) kullanılacağını söylemez; bu bölüm son kaydı (yaş sınırsız) alır.
...
- **`plateau_insights.side` kolonu `CHECK` içermiyor** (03 §1.7'de `TEXT NOT NULL DEFAULT 'both'`); `set_logs.side` ile aynı `CHECK (side IN ('both','left','right'))` eklenmeli.
...
3. **Zod sınırı vs. DB CHECK:** 02 §11.3 ve 03 §4 `z.number().positive().max(300)` (300 dahil) der; 03 DDL `final_value_cm < 300` ve `weight_kg < 400` (hariç). Tam 300 / 400 Zod'dan geçip DB'de reddedilir. Bu bölüm `.lt(300)` / `.lt(400)` kullandı; 02/03 metni güncellenmeli.
