...
| AT-14, AT-15, AT-16 | Uygulanır | Aynı ZIP; export tarayıcı indirmesi (`deliverExport`; ekran "İndirme başlatıldı" der), import dosya seçici (`readPickedBytes`); staging/pre-import görüntüleri şifreli görüntü deposunda (`RoutedBlobStore`); fotoğraflar ve "Geri al" kaydı (IndexedDB `blobs`/`meta`) aynı anahtarla şifreli (`EncryptedKvStore`). Web yığınında Node'da `test/webBackup.test.ts` koşar: (1) sıfır fotoğraf round-trip (dizin işareti), (2) fotoğraflı round-trip, (3) bozuk ZIP → mevcut görüntü ve fotoğraflar bayt bayt korunur (R95.7). |
...
Web'e özgü ek denetimler (kabul sayacına dahil değil; ADR-013 Doğrulama): ikinci sekme `DbOpenError` ile durur; site verisi temizlenince onboarding + indirilmiş ZIP'ten geri yükleme; kalıcı depolama durumu (`requestPersistentStorage`) Ayarlar'da görünür; `scripts/check-bundle.mjs web` — yasak izler: `NodeSqliteProvider`, `nodeSqliteDriver`, `@journeyapps/sqlcipher`, `expo-sqlite`, `expoSqliteDriver`, `WHEN_UNLOCKED_THIS_DEVICE_ONLY`, `ExpoSecureStore`; zorunlu izler: `getRowsModified`, `sqlJsDriver`, `AES-GCM`, `v90.web.dbkey`, `V90E` (`PRAGMA key` metni yasaklanmaz: ortak `SqliteDatabaseProvider`'da durur ve yalnızca `keyManager` verildiğinde gönderilir; web sağlayıcısı anahtarsızdır); web'de alınan ZIP'in Android uygulamasında içe aktarılması ve tersi. Web'e özgü otomatik test: `test/webBackup.test.ts` (AT-14/15/16'nın web yığını karşılığı).
